subroutine CorrectedScaGradPtoP(delF_a,delF_b,a,b,F_a,F_b,dwdx, mass_a, mass_b, rho_a, rho_b, &
    ...
    call ScalarGradientPtoP(delF_a,delF_b,a,b,F_a,F_b,Cdwdx_a, Cdwdx_b, mass_a, mass_b, rho_a, rho_b, dim, grad_type)
    
endsubroutine
    
    
    
subroutine CorrectedScaGradPtoB(delF_a,a,s,F_a,F_b,del_gamma_as,&
                    & gamma_cont_a, gamma_discrt_a, gamma_mat_a, gamma_mat_inv_a, xi1_mat_inv_a, &
                    & FS_a,dim, CF_ID, grad_type) 

    !vector divergence for particle itneracting with boundary is calculated, by first correctign the kernel gradient

    implicit none
    integer(4), intent(in) :: dim, CF_ID, grad_type,a,s
    ...
    call ScalarGradientPtoB(delF_a,a,s,F_a,F_b,Cdgmas, dim, grad_type)
