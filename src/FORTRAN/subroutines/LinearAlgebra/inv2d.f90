...
subroutine inv2d(B,A, diag_value)
    ...
    det_val=A(2,2)*A(1,1)-A(1,2)*A(2,1)    
    
    do i=1,2
        do j=1,2
            B(i,j)=A_cofactor(i,j)/det_val
		enddo
    enddo
    
    det_val=B(2,2)*B(1,1)-B(1,2)*B(2,1)    
    
    if (isNAN(det_val)) then
        write(*,*) 'Warning : matrix is non invertible, caution while using the inverse directly'
        B(:,:)=0.D0
        B(1,1)= 1.D0/diag_value
        B(2,2)= 1.D0/diag_value
    endif
