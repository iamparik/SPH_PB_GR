...
!****************************************************************************
    ...
    maxn= (npw*npl) + (npw+npl+4)*2*(3)*e_divide !+ npw*npl! 3 for 3 points/particles per edge
    ...
    write(*,*) "npw =" , npw, "npl =", npl, "dx_r=", dx_r , "with width=", width
    ...
    width1 = x(2,k) + dx_r/2.D0 + dx_r*scale_k*dble(nint(hsml_const/dx_r))*2.D0
...
! Create particles with no mass to capture value
    ...
    do i=1,ke
        ...
        if((i .gt. keBottom) .and. (i .le. keRight)) then
            etype(i)=etype_periodic!etype_thermal_neumann    
            pBC_edges(1,1)=i
        endif
        
        
        if((i .gt. keRight) .and. (i .le. keTop)) etype(i)=etype_FarWall!etype_thermal_neumann
        
        if((i .gt. keTop) .and. (i .le. keLeft)) then
            etype(i)=etype_periodic!etype_thermal_neumann 
            pBC_edges(2,1)=i
        endif
