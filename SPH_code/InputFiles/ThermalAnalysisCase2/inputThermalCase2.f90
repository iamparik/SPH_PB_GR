...
!****************************************************************************
    ...
    use config_geometry, only: npw, npl, dx_r, bt_dx_r, length, width, hsml_const, &
        & rho_init, mu_const, g_const, c_sound,BILtype 
    ...
    integer(4) i,j,s, k,ke ,e_divide,disorderedParticleArray
    real(8) scale_k, mn, length1, width1, ecf 
    integer(4) ke_out, keTop,keBottom, keRight, keLeft!case specific width
    integer(4) npa
...
! Create particles with no mass to capture value
    ...
    do i=1,e_divide*npw
        ke=ke+1
        xn(1,ke)=length1 
        xn(2,ke)=dble(i-1)*dx_r/dble(e_divide) 
    enddo
    ...
    do i=1,e_divide*npw
        ke=ke+1
        xn(1,ke)=0.D0 
        xn(2,ke)=width1 - dble(i-1)*dx_r/dble(e_divide)
    enddo
    ...
    do i=1,ke
        ...
        if(i .le. keBottom) etype(i)=etype_thermal_dirichlet
            
        if((i .gt. keBottom) .and. (i .le. keRight)) etype(i)=etype_thermal_neumann    
        
        if((i .gt. keRight) .and. (i .le. keTop)) etype(i)=etype_thermal_neumann
        
        if((i .gt. keTop) .and. (i .le. keLeft)) etype(i)=etype_thermal_neumann    
