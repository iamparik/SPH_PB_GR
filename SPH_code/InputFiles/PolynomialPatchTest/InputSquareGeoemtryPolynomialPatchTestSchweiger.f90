...
!****************************************************************************
    ...
    use config_parameter, only: SPH_dim, etype_SolidWall1
    ...
    real(8) ke_out!case specific width
    integer(4) npa
    real(8),DIMENSION(:,:),ALLOCATABLE :: xn
    
    npw=100
    dx_r = width/(dble(npw) + bt_dx_r*2.D0)
    npl =nint(length/dx_r)
    
    hsml_const= 1.D0*dx_r 
    ...
    do i=1,npl
        do j=1,npw
            k=k+1
            x(1,k)= dble(i-1)*dx_r + dx_r/2.D0 +2.D0
            x(2,k)= dx_r*dble(j-1) + dx_r/2.D0 +2.D0                                         
            vx(1,k)=0.D0 
            vx(2,k)=0.D0 
...
! Create particles with no mass to capture value
    ...
    do i=1,e_divide*npl
        ke=ke+1
        xn(1,ke)=dble(i-1)*dx_r/dble(e_divide) +2.D0
        xn(2,ke)=0.D0 +2.D0
    enddo
    ...
    do i=1,e_divide*npw
        ke=ke+1
        xn(1,ke)=length1 
        xn(2,ke)=dble(i-1)*dx_r/dble(e_divide) +2.D0
    enddo
    ...
    do i=1,ke
        ...
        etype(i)=etype_SolidWall1      
        
        !if ( (i .gt. ke/2) .and. (i .le. int(3.D0*dble(ke)/4.D0)) ) etype(i)=etype_SolidWall2
