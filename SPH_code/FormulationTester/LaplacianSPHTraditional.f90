...
subroutine LaplacianSPHTraditional(lapf, f0, G, x, gma, SPH_dim, niac, pair_i, pair_j, dwdx, mass, rho, itype, ntotal)
    implicit none
    integer(4), intent(in):: ntotal, SPH_dim, niac
    integer(2), intent(in):: itype(ntotal)
    real(8), intent(in):: f0(ntotal), gma(ntotal), mass(ntotal), rho(ntotal)
    ...
    !The below is being tessted, but above works for small arrays
    !
    !integer(4), intent(in):: ntotal, SPH_dim, niac
    !integer(2), intent(in), allocatable:: itype(:)
    !real(8), intent(in), allocatable:: f0(:), gma(:), mass(:), rho(:), G(:)
    !real(8), intent(in), allocatable:: x(:,:), dwdx(:,:)
    !integer(4), intent(in), allocatable:: pair_i(:), pair_j(:)
    !real(8), intent(inout), allocatable:: lapf(:)    
    !integer(4)::k,a,b,s,d
    !real(8) ::  delf, r_ab, lap_coeff
    !
