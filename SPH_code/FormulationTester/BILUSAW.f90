...
subroutine BILUSAW(lapf, df0, dfs0, gma, SPH_dim, eniac, epair_a, epair_s, dgmas, etype, &
                & etotal, ntotal,etype_periodic, Neumann, Dirichlet, bdryVal, bdryValDim)
    ...
    real(8), intent(in)::gma(ntotal), bdryVal(bdryValDim) 
    real(8), intent(in):: df0(SPH_dim,ntotal) ,dfs0(SPH_dim,etotal)
     ...
     do k= 1, eniac
        ...
        elseif(mod(etype(s),etype_periodic) .eq. Dirichlet) then
            if(WallBoundaryLayer) then
                lap_coeff=2.D0*bdryVal(k)*norm2(dgmas(:,k))
                call fncnApproxOperator(lapf(a),lap_coeff,1.D0,1.D0,-1.D0/gma(a))
            else
                do d=1,SPH_dim
                    lap_coeff= (df0(d,a)+dfs0(d,s))*dgmas(d,k)
                    call fncnApproxOperator(lapf(a),lap_coeff,1.D0,1.D0,-1.D0/gma(a))
                enddo
            endif
