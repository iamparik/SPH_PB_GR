...
subroutine CorrectionFactorMatrixInversion
    ...
    if( .NOT. allocated(xi_cont_mat_inv)) allocate(xi_cont_mat_inv(SPH_dim,SPH_dim,ntotal))
    xi1_mat_inv=0.D0
    gamma_mat_inv=0.D0
    xi_cont_mat_inv=0.D0
    ...
    if(SPH_dim .eq. 2) then
        do a=1,ntotal
            ...
            if((itype(a) .le. itype_real_max) .and. (itype(a) .ge. itype_real_min)) then
                 ...
                 call inv2d(xi_cont_mat_inv(:,:,a), xi_cont_mat(:,:,a))
             endif
        enddo
    else
