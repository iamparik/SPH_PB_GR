...
!****************************************************************************      
subroutine  output_time_flow(itimestep,dt)
    ...
    use particle_data ,   only: x, mass, rho, p, vx, nedge_rel_edge, &
        & ntotal,etotal, itype, etype, gamma_cont, gamma_discrt,&
        &  max_vel, KE,PE, TE, del_gamma,delcAvg,delCMax, delCL2, delC
...
!------------------------------------------------------------------------
    ...
    if (itimestep .eq. 1) then
        ...
        !open (1, file = x2name)
        !write(1,*) 'variables = time, KE, PE, TE, u_max , delcAvg, delCMax'
        !close(1)
    elseif (mod(itimestep,save_step).eq.0) then
        write(x2name, '(A,A)') dataOutputPath,'/energy.dat'
        
        open (1, file = x2name,  position='append')
        
        do i=1,save_step
            write(1,1002) dble(itimestep-save_step+i)*dt*sqrt(g_const/hydroStaticHeight), KE(i), PE(i), TE(i), max_vel(i), delcAvg(i), delCMax(i), delCL2(i)
        enddo
...
! Export all Ghost particles/ponts.
    ...
    do i=1, ntotal
        if(itype(i) .eq. 0) then
            ...
            write(1,1001) (x(d, i), d=1,SPH_dim),0,0, (0, d = 1, SPH_dim), 0, gamma_cont(i), gamma_discrt(i), (del_gamma(d, i), d = 1, SPH_dim), (0, d = 1, SPH_dim)       
            text_print=.false.
        endif
    enddo
...
1001 format(13(e22.10,2x))    ! i am using Maximum, it was 9 before
