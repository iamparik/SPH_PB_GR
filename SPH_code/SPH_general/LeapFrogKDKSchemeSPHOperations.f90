!****************************************************************************
!
!  SUBROUTINE: LeapFrogKDKSchemeSPHOperations
!
...
use particle_data, only:itimestep, w_aa, w, dwdx, &
        ...
        & dynamicProblem, rho_prev,drho, b_MLS
...
if (itimestep .eq.0) call output_initial_config
...
!Call a time integration scehme/ operator test function to make various calulations
    if(NumericalSimCase .eq. 3 ) then
        call LeapFrogKDKIntegration_fluid(itimestep,dt)
    else
        call LeapFrogKDKIntegration_fluid(itimestep,dt)
    endif
