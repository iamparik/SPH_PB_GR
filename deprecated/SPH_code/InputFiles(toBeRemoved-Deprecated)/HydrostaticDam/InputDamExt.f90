...
!****************************************************************************
    ...
    use config_parameter, only: SPH_dim,etype_periodic, etype_SolidWall1, etype_SolidWall2, &
        ...
        & DataConfigInputPath, itype_virtual, itype_real_min
    use config_geometry, only: dx_r, hsml_const,hydrostaticHeight, &
    ...
    real(8) tempType
    integer(4) k,d,s,ke,periodicPairs,i
    ...
    do while (.not.eof(1))
        ! I might have to modify below to strictly read as integers
          read(1,*) maxn, maxnv, maxedge, periodicPairs
    enddo
    close(1)
    ...
    do while (.not.eof(1))
        ...
        elseif(etype(s) .eq. etype_periodic) then             
            if( .not. allocated(pBC_edges)) then
                write(*,*) "periodic BCS not yet available for CAD data import"
                pause
                allocate(pBC_edges(2,periodicPairs))
                allocate(tangent_pBC(SPH_dim,2,periodicPairs))                
            endif          
    ...
    ALLOCATE(surf_norm(SPH_dim, etotal))    
    do s = 1, etotal   
    ...
    ALLOCATE(nedge_rel_edge(etotal))
    ...
    do k=1,ntotal
        ...
        rho(k)=rho_init*((0.7D0*9.81D0*(hydroStaticHeight-x(2,k))/(c_sound**2)+1.D0)**(1.D0/0.7D0))
        p(k)=(rho_init*c_sound**2/0.7D0)*((rho(k)/rho_init)**0.7D0-1.D0) 
        mass(k)=rho(k)*vol(k)
        hsml(k)=hsml_const
        mu(k)=mu_const
        p(k)=0.D0
