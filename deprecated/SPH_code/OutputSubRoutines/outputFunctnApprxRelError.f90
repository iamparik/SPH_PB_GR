...
subroutine  outputFncnApprx(n,x, fncn0, fncn, dfncn0, dfncn, lap_fncn0, lap_fncn, nedge_rel_edge, edge, etotal, SPH_dim, ntotal,itype,etype,dataOutputPath)
     ...
     use particle_data, only: mass, rho
    implicit none
!----------------------------------------------------------------------           
...
! for 3D output 
    if(SPH_dim.eq.3)    write(1,'(A)') 'variables ="x", "y", "z", "f0", "f", "RE(f)", "df0(x)", "df0(y)", "df0(z)", "df(x)", "df(y)", "df(z)", &
                                &  "REdf(x)", "REdf(y)", "REdf(z)","d2f0","d2f", "REd2f" '
! for 2D output
    if(SPH_dim.eq.2)    write(1,'(A)') 'variables="x", "y", "f0", "f", "RE(f)", "df0(x)", "df0(y)","df(x)", "df(y)", "REdf(x)", "REdf(y)",  "d2f0","d2f", "REd2f"  '
! for 1D output
    if(SPH_dim.eq.1)    write(1,'(A)') 'variables = "x", "f0", "f","RE(f)", "df0(x)", "df(x)","REdf(x)", "d2f0","d2f", "REd2f"'
...
! First export data of all real particles ( not on boundary)
    ...
    do a=1,ntotal
        if((itype(a) .le. itype_real_max) .and. (itype(a) .gt. itype_real_min)) then
            ...
            write(1,1001) (x(d, a), d=1,SPH_dim), fncn0(a), fncn(a),abs(1-fncn(a)/fncn0(a)), (dfncn0(d, a), d=1,SPH_dim), &
                        & (dfncn(d, a), d=1,SPH_dim), (abs(1-dfncn(d,a)/dfncn0(d,a)), d=1,SPH_dim) ,lap_fncn0(a), lap_fncn(a),&
                        & abs(1-lap_fncn(a)/lap_fncn0(a))
