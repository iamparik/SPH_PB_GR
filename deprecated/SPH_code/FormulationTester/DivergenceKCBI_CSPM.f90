!
!  SUBROUTINE: DivergenceKCBI_CSPM
...
subroutine DivergenceKCBI_CSPM( d1f, f0, Xi_inv, SPH_dim, niac, pair_i, pair_j, dwdx, eniac, epair_a, dgmas, mass, rho, itype, ntotal, num_edges)
    implicit none
    ...
    ! Call the interpolation operation for first order derivative of a function
     ...
     do k = 1,niac
        ...
        do d=1,SPH_dim 
            ...
            call fncnApproxOperator(d1f(a),f0(d,b),mass(b),rho(b),CdwdxA(d))
            call fncnApproxOperator(d1f(b),f0(d,a),mass(a),rho(a),CdwdxB(d))
    ...
    ! [ξ_a^-1 ]_ij ∑_s∫f_a W_(as' ) n_(s_j ) dS'
     do k= 1, eniac
        ...
        do d=1,SPH_dim 
            ...
            call fncnApproxOperator(d1f(a),-f0(d,a),1.D0,1.D0,Cdgmas(d))
