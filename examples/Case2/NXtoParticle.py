...
def bulkCase2D(input_file_path, output_file_path):
    ...
    with open(output_file_path, 'w') as output_file:
        ...
        for tria_element in ctria3Elements:
            ...
            QAreas.append(area)
            output_file.write(f"{int(tria_element[0]):<10} {centroid[0]/unitConversion:<20.10f} {centroid[1]/unitConversion:<20.10f} {area/(unitConversion**2):<20.10f}\n")
        ...
        for quad_element in cquad4Elements:
            ...
            QAreas.append(area)
            output_file.write(f"{int(quad_element[0]):<10} {centroid[0]/unitConversion:<20.10f} {centroid[1]/unitConversion:<20.10f} {area/(unitConversion**2):<20.10f}\n")
...
def bulkBdryCase2D(input_file_path,output_bdryfile_path, output_cartesianfile_path, dx, bdry_tolerance, parallelBool):
    ...
    with open(output_bdryfile_path, 'w') as output_file:
        ...
        for oneD_element in oneDElements_Bulkbdry:
            ...
            output_file.write(f"{int(oneD_element[0]):<10} {vertices[0][1]/unitConversion:<20.10f} {vertices[0][3]/unitConversion:<20.10f} {vertices[1][1]/unitConversion:<20.10f} {vertices[1][3]/unitConversion:<20.10f}\n")
    ...
    totalNum2DElements_Cartesian = fillDomainWParticle2D(x_min, x_max, dx, bdryPolygon, output_cartesianfile_path, bdry_tolerance, parallelBool)
...
def domainBdryCase2D(input_file_path,output_file_path):
    # Regular expressions for GRID and CQUAD4 lines
    ...
    for label, color in int_to_color.items():
        plt.text(0.2, 0.9, f'dx_r = {dx_r}', fontsize=10, transform=plt.gcf().transFigure)
        plt.text(0.8, 0.9, f'unit {int(unitConversion)} : 1', fontsize=10, transform=plt.gcf().transFigure)
        if (label <= bdryType):
    ...
    plt.savefig('data_geo_config/domainBdry.png') 
    #plt.show()
...
def fillDomainWParticle2D(x_min, x_max, dx,bdryPolygon, output_file_path, bdry_tolerance,  parallelBool):
    ...
    with open(output_file_path, 'w') as output_file:
        for points in inside_points:
            ...
            output_file.write(f"{Qnumber:<10} {points[0]/unitConversion:<20.10f} {points[1]/unitConversion:<20.10f} {avgAreaCartesian/(unitConversion**2):<20.10f}\n")
...
def main():
    input_file_path_SA = "inputCAD/bulk2D.dat"
    output_file_path_SA = "data_geo_config/input_particles_bulkMesh.dat"

    #------------------------------------------------------------------------------------------
    ############# First convert area mesh generated in CAD to 2D particles #####################
    if os.path.exists(input_file_path_SA):
        print(" ****************************\n bulk.dat file is processed to \n generate particles \n **********************")
        totalNum2DElements,total2DArea= bulkCase2D(input_file_path_SA,output_file_path_SA)
    else :
    ...
    input_file_path_Bulkbdry = "inputCAD/bulkBdry1D.dat"
    output_file_path_Bulkbdry = "data_geo_config/input_bulkBdryEdge.dat"
    ...
    input_file_path_bdry = "inputCAD/domainBdry1D.dat"
    output_file_path_bdry = "data_geo_config/input_domainBdryEdge.dat"

    if os.path.exists(input_file_path_bdry):
        print(" **************************** \n domainBdry.dat file is processed to : \n to process boundary elements of domain \n ****************************")
        totalNum1DElements=domainBdryCase2D(input_file_path_bdry,output_file_path_bdry)
    else:
        totalNum1DElements = 0
    #------------------------------------------------------------------------------------------
    ######### Create Sim File ############################################################################################################

    output_file_path_simSize = "data_geo_config/input_param_SimSize.dat"


    with open(output_file_path_simSize, 'w') as output_file:
        output_file.write(f"{totalNum2DElements:<10} {totalNum2DElements_Cartesian:<10} {totalNum1DElements_Bulkbdry:<10} {totalNum1DElements:<10} {total2DArea/(unitConversion**2):<20.30f}\n")
